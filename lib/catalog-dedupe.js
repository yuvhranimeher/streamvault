...
function mergeSeriesSeasons(...groups) {
  const seasons = new Map();
  const entries = value => {
    if (Array.isArray(value)) return value.map((season, index) => [season?.season ?? season?.seasonNumber ?? index + 1, season]);
    return value && typeof value === 'object' ? Object.entries(value) : [];
  };
  for (const group of groups) {
    for (const [rawSeason, seasonValue] of entries(group)) {
      const number = Number(String(rawSeason ?? '').match(/\d+/)?.[0] ?? seasonValue?.season ?? seasonValue?.seasonNumber ?? 1);
      const key = Number.isInteger(number) && number >= 0 ? number : 1;
      const episodeMap = seasons.get(key) || new Map();
      const episodes = Array.isArray(seasonValue)
        ? seasonValue
        : (Array.isArray(seasonValue?.episodes) ? seasonValue.episodes : (Array.isArray(seasonValue?.items) ? seasonValue.items : []));
      episodes.forEach((episode, index) => {
        const episodeNumber = Number(episode?.episode ?? episode?.number ?? episode?.episodeNumber ?? index + 1) || index + 1;
        const existing = episodeMap.get(episodeNumber);
        if (!existing) episodeMap.set(episodeNumber, episode);
        else {
          const best = qualityScore(episode) > qualityScore(existing) ? episode : existing;
          const other = best === episode ? existing : episode;
          episodeMap.set(episodeNumber, {
            ...other,
            ...best,
            sources: mergeSources(existing.sources || [], episode.sources || [], sourceForItem(existing), sourceForItem(episode))
          });
        }
      });
      seasons.set(key, episodeMap);
    }
  }
  return [...seasons.entries()].sort((a, b) => a[0] - b[0]).map(([season, episodes]) => ({
...
function mergeCatalogItems(a = {}, b = {}) {
  ...
  if (a.seasons || b.seasons) merged.seasons = mergeSeriesSeasons(a.seasons, b.seasons);
