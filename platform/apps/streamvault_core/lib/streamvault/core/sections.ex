defmodule StreamVault.Core.Sections do
  ...
  @definitions [
    ...
    %{
      key: "indian",
      row_id: "indianRow",
      title: "Indian Movies & Drama",
      rule: {:contains, ~w(hindi bangla bengali tamil telugu bollywood india)}
    },
    %{
      key: "anime",
      row_id: "animeRow",
      title: "Anime",
      rule: {:contains, ~w(anime japanese animation)}
    },
    %{
      key: "koreanDrama",
      row_id: "koreanRow",
      title: "Korean Drama",
      rule: {:contains, ~w(korean korea kdrama)}
    },
    %{
      key: "horrorNights",
      row_id: "horrorRow",
      title: "Horror Nights",
      rule: {:contains, ~w(horror ghost demon haunted)}
    },
    %{
      key: "cyberpunkScifi",
      row_id: "scifiRow",
      title: "Cyberpunk & Sci-Fi",
      rule: {:contains, ~w(scifi science space cyberpunk futuristic alien)}
    },
    %{
      key: "documentaryVault",
      row_id: "documentaryRow",
      title: "Documentary Vault",
      rule: {:contains, ~w(documentary biography history nature)}
    },
  ...
  def home_feed(items, limit) do
    rows =
      ...
      |> Enum.map(fn definition ->
        selected =
          items |> Enum.filter(&matches?(&1, definition.rule)) |> rank() |> Enum.take(limit)
  ...
  defp matches?(%Media{year: year}, {:year_at_least, minimum}) when is_integer(year),
    do: year >= minimum

  defp matches?(%Media{rating: rating}, {:rating_at_least, minimum}) when is_number(rating),
    do: rating >= minimum
  ...
  defp rank(items) do
    Enum.sort_by(items, fn item ->
      {if(item.poster || item.backdrop, do: 0, else: 1), -(item.rating || 0), -(item.year || 0),
       item.title}
    end)
  end
end
