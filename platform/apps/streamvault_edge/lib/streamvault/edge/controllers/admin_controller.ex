defmodule StreamVault.Edge.Controllers.AdminController do
  ...
  def reload_catalog(conn, _params) do
    if authorized?(conn) do
      case Catalog.reload() do
        {:ok, metadata} ->
          Response.ok(conn, %{ok: true, catalog: metadata})

        {:error, reason} ->
          Response.error(conn, 500, "catalog_reload_failed", "Catalog reload failed", %{
  ...
  defp authorized?(conn) do
    ...
    is_nil(configured) or
      (is_binary(supplied) and Plug.Crypto.secure_compare(configured, supplied))
  end
end
