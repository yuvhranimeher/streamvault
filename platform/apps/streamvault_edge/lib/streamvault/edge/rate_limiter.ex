defmodule StreamVault.Edge.RateLimiter do
  ...
  @spec allow?(String.t(), pos_integer(), pos_integer()) :: {:allow, non_neg_integer()} | {:deny, non_neg_integer()}
  ...
  def handle_call({:allow, key, limit, window_ms}, _from, state) do
    ...
    result = if count <= limit, do: {:allow, max(limit - count, 0)}, else: {:deny, max(reset_at - now, 0)}
    {:reply, result, state}
  end
  ...
  def handle_info(:sweep, state) do
    now = System.monotonic_time(:millisecond)
    @table |> :ets.tab2list() |> Enum.each(fn {key, _count, reset_at} -> if reset_at <= now, do: :ets.delete(@table, key) end)
    Process.send_after(self(), :sweep, :timer.minutes(5))
    {:noreply, state}
  end
end
