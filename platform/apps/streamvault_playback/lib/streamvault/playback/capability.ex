defmodule StreamVault.Playback.Capability do
  ...
  def from_map(map) do
    ...
    struct(defaults, %{
      ...
      video_codecs: list(value(map, "videoCodecs") || value(map, "video_codecs"), defaults.video_codecs),
      audio_codecs: list(value(map, "audioCodecs") || value(map, "audio_codecs"), defaults.audio_codecs),
      max_height: integer(value(map, "maxHeight") || value(map, "max_height")) || defaults.max_height,
      max_bitrate: integer(value(map, "maxBitrate") || value(map, "max_bitrate")) || defaults.max_bitrate,
      supports_hls: boolean(value(map, "supportsHls") || value(map, "supports_hls"), defaults.supports_hls),
      supports_range: boolean(value(map, "supportsRange") || value(map, "supports_range"), defaults.supports_range),
  ...
  def infer(user_agent) do
    ...
    cond do
      String.contains?(normalized, ["iphone", "ipad", "apple tv"]) ->
        %__MODULE__{device: "apple", user_agent: user_agent, containers: ["mp4", "hls"], video_codecs: ["h264", "hevc"], supports_hls: true}

      String.contains?(normalized, "android") ->
        %__MODULE__{device: "android", user_agent: user_agent, containers: ["mp4", "webm", "hls"], video_codecs: ["h264", "vp9", "av1"], supports_hls: true}
  ...
  defp list(value, _default) when is_list(value), do: Enum.map(value, &String.downcase(to_string(&1)))
