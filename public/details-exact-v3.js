...
/* SV_CANONICAL_SERIES_DETAIL_V3 */
(function(){
  if(window.__svCanonicalSeriesDetailV3)return;
  window.__svCanonicalSeriesDetailV3=true;
  ...
  function episodeCount(item){
    return Object.values(item?.seasons || {}).reduce((count,list)=>count+(Array.isArray(list)?list.length:0),0);
  }
  ...
  async function requestCanonicalSeries(show,key){
    ...
    const response=await fetchWithTimeout(`/api/series/${encodeURIComponent(key)}?${params.toString()}`,{},15000);
    if(!response?.ok)return null;
  ...
  function applyCanonicalDetail(key,show,full){
  ...
  openSeriesDetail=async function(key){
    ...
    if(immediate){
      applyCanonicalDetail(key,show,immediate);
      showSeriesDetail(show);
      return;
    }
    showSeriesDetail(show);
    try{
      ...
      applyCanonicalDetail(key,show,full);
      showSeriesDetail(show);
    }catch(error){
