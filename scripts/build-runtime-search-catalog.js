...
function catalogRoots() {
  ...
  // Useful when this script is run from the crawler Mac rather than the Windows backend.
...
function discoverSources() {
  ...
  for (const root of catalogRoots()) {
    ...
    if (fs.existsSync(storage)) {
      for (const name of fs.readdirSync(storage)) {
        const candidate = path.join(storage, name, 'files.jsonl');
        add(jsonl, candidate);
      }
    }
  ...
  // Optional explicit sources. Use semicolon-separated paths on Windows/macOS/Linux.
