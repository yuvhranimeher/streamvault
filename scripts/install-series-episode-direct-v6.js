...
const MARKER = 'SV_SERIES_EPISODE_DIRECT_V6';
const OLD_MARKER = '// SV_SERIES_EPISODE_DIRECT_V5';
...
function install() {
  ...
  let start = source.lastIndexOf(OLD_MARKER, detailAt);
  if (start < 0) start = source.lastIndexOf('function svDirectEpisodeKey(value) {', detailAt);
  if (start < 0) throw new Error('Could not locate installed direct episode V5 block');

  const block = `// ${MARKER}
const svDirectEpisodeResponseCacheV6 = new Map();
let svDirectEpisodeIndexV6 = null;
  ...
  const put = (seasonNo, rawEpisodes) => {
    ...
    const normalized = list
      ...
      .map((raw, index) => {
        ...
        let displayTitle = '';
        for (const value of [ep.epTitle, ep.title, ep.name]) {
...
function svDirectBuildEpisodeIndexV6() {
  if (svDirectEpisodeIndexV6) return svDirectEpisodeIndexV6;
  const index = new Map();
  const candidates = [];
  try {
    loadMassiveCatalog();
    ...
    if (SV_DETAIL_VERBOSE) console.warn('[Episodes direct V6] massive catalog:', error.message);
  }
  try {
    const ftp = getCachedSeries();
    if (Array.isArray(ftp)) candidates.push(...ftp);
  } catch (error) {
    if (SV_DETAIL_VERBOSE) console.warn('[Episodes direct V6] FTP catalog:', error.message);
  }
...
app.get('/api/series/episodes-direct', (req, res) => {
  try {
    ...
    const cached = svDirectEpisodeResponseCacheV6.get(responseKey);
    if (cached) {
      res.setHeader('Cache-Control', 'public, max-age=3600');
      res.setHeader('X-StreamVault-Episode-Authority', 'v6-cache');
      return res.json(cached);
    }

    const index = svDirectBuildEpisodeIndexV6();
    ...
    const payload = {
      ...
      hasStream: true,
      isSummary: false,
      authority: 'series-episode-direct-v6'
    };

    svDirectEpisodeResponseCacheV6.set(responseKey, payload);
    res.setHeader('Cache-Control', 'public, max-age=3600');
    res.setHeader('X-StreamVault-Episode-Authority', 'v6-index');
    return res.json(payload);
  } catch (error) {
    console.error('[Episodes direct V6] failed:', error.stack || error.message);
...
`;
  ...
  console.log('[Startup] Installed indexed series episode authority V6.');
  return true;
}
...
if (require.main === module) {
  try { install(); }
    ...
    console.error('[Series episode direct V6] install failed:', error.stack || error.message);
    process.exitCode = 1;
  }
}
