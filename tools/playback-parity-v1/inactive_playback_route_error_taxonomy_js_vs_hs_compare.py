#!/usr/bin/env python3
"""Compare inactive playback route error taxonomy JS and Haskell shadows."""
...
def report_path() -> Path:
    ...
    return TOOL_DIR / f"inactive-playback-route-error-taxonomy-js-vs-hs-report-{stamp}.txt"
...
def compile_haskell(ghc: str, tmp_dir: Path) -> tuple[Path | None, str, list[str]]:
    binary = tmp_dir / "InactivePlaybackRouteErrorTaxonomy"
...
def run_haskell(binary: Path) -> tuple[Any | None, str, list[str]]:
    ...
    if result.returncode != 0:
        detail = result.stderr.strip() or result.stdout.strip() or "Haskell error taxonomy shadow failed"
        return None, "failed", [detail]
...
def run_javascript(node: str) -> tuple[Any | None, str, list[str]]:
    ...
    if result.returncode != 0:
        detail = result.stderr.strip() or result.stdout.strip() or "JS error taxonomy shadow failed"
        return None, "failed", [detail]
...
def main() -> int:
    ...
    if not node:
        failures.append("node unavailable: load Node.js before running error taxonomy comparator")
    if not ghc:
        failures.append("ghc unavailable: load GHC before running error taxonomy comparator")
    ...
    if node and ghc and not failures:
        with tempfile.TemporaryDirectory(prefix="sv-error-taxonomy-") as tmp:
    ...
    if haskell_output is not None and javascript_output is not None:
        ...
        if compare_status == "failed":
            failures.append("normalized inactive error taxonomy JS output differs from normalized Haskell output")

    ok = not failures and compare_status == "passed"
    lines = [
        f"Status: {'PASS' if ok else 'FAIL'}",
        "mode: read-only inactive playback route error taxonomy JS vs Haskell comparison",
        "server_started: no",
        "network_called: no",
        "ffmpeg_started: no",
