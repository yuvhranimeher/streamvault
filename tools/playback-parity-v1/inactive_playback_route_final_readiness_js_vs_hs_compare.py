#!/usr/bin/env python3
"""Compare inactive playback route final readiness JS and Haskell shadows."""
...
FIXTURE_PATH = TOOL_DIR / "inactive-playback-route-final-readiness-fixtures.json"
...
def report_path() -> Path:
    ...
    return TOOL_DIR / f"inactive-playback-route-final-readiness-js-vs-hs-report-{stamp}.txt"
...
def compile_haskell(ghc: str, tmp_dir: Path) -> tuple[Path | None, str, list[str]]:
    binary = tmp_dir / "InactivePlaybackRouteFinalReadiness"
...
def run_haskell(binary: Path) -> tuple[Any | None, str, list[str]]:
    ...
    if result.returncode != 0:
        detail = result.stderr.strip() or result.stdout.strip() or "Haskell final readiness shadow failed"
        return None, "failed", [detail]
...
def run_javascript(node: str) -> tuple[Any | None, str, list[str]]:
    ...
    if result.returncode != 0:
        detail = result.stderr.strip() or result.stdout.strip() or "JS final readiness shadow failed"
        return None, "failed", [detail]
...
def main() -> int:
    ...
    if not node:
        failures.append("node unavailable: load Node.js before running final readiness comparator")
    if not ghc:
        failures.append("ghc unavailable: load GHC before running final readiness comparator")
    ...
    if node and ghc and not failures:
        with tempfile.TemporaryDirectory(prefix="sv-final-readiness-") as tmp:
    ...
    if haskell_output is not None and javascript_output is not None:
        ...
        if compare_status == "failed":
            failures.append("normalized final readiness JS output differs from normalized Haskell output")

    component_count = 0
    if isinstance(javascript_output, dict):
        component_count = int(javascript_output.get("componentCount") or 0)

    ok = not failures and compare_status == "passed"
    lines = [
        f"Status: {'PASS' if ok else 'FAIL'}",
        "mode: read-only inactive playback route final readiness JS vs Haskell comparison",
        "server_started: no",
        "network_called: no",
        "ffmpeg_started: no",
