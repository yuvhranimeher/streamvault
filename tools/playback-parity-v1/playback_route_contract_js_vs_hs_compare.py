#!/usr/bin/env python3
"""Compare JS and Haskell playback route contract shadow planners."""
...
HS_PATH = TOOL_DIR / "PlaybackRouteContractShadow.hs"
JS_PATH = TOOL_DIR / "playback_route_contract_shadow_js.js"
REPORT_PATH = TOOL_DIR / "playback-route-contract-js-vs-hs-report-20260612-182035.txt"
...
def compile_haskell(ghc: str, tmp_dir: Path) -> tuple[Path | None, str, list[str]]:
    binary = tmp_dir / "PlaybackRouteContractShadow"
...
def run_haskell(binary: Path) -> tuple[Any | None, str, list[str]]:
    ...
    if result.returncode != 0:
        detail = result.stderr.strip() or result.stdout.strip() or "Haskell route contract planner failed"
        return None, "failed", [detail]
...
def run_javascript(node: str) -> tuple[Any | None, str, list[str]]:
    ...
    if result.returncode != 0:
        detail = result.stderr.strip() or result.stdout.strip() or "JS route contract planner failed"
        return None, "failed", [detail]
...
def main() -> int:
    ...
    if not node:
        failures.append("node unavailable: install/load Node.js before running route contract comparator")
    if not ghc:
        failures.append("ghc unavailable: install/load GHC before running route contract comparator")
    ...
    if node and ghc:
        with tempfile.TemporaryDirectory(prefix="sv-route-contract-") as tmp:
    ...
    if haskell_output is not None and javascript_output is not None:
        ...
        if compare_status == "failed":
            failures.append("normalized route contract JS output differs from normalized Haskell output")

    ok = not failures and compare_status == "passed"
    lines = [
        f"Status: {'PASS' if ok else 'FAIL'}",
        "mode: read-only playback route contract JS vs Haskell comparison",
        "server_started: no",
        "network_called: no",
        "ffmpeg_started: no",
